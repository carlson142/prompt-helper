...
{
  /* ЗБЕРІГАННЯ ДАНИХ З MULTI STEP FORM */
}
...
interface MultiFormData {
  dataForPrompt: IMSFData | null;
  setMultiFormData: (someData: IMSFData) => void;
}

export const useMultiStepFormData = create<MultiFormData>()((set) => ({
  dataForPrompt: null,
  setMultiFormData: (someData: IMSFData) => set({ dataForPrompt: someData }),
}));
